...
-- 执行日期: 2026-04-15
-- 说明: 补充全栈 A + 全栈 B 新增实体字段 + P0/P2 Bug 修复

USE opspilot;

-- t_server 新增字段（P0 Bug-002 修复）
ALTER TABLE t_server ADD COLUMN IF NOT EXISTS jdk_versions VARCHAR(500) DEFAULT NULL COMMENT 'JDK 版本列表（JSON）';
ALTER TABLE t_server ADD COLUMN IF NOT EXISTS node_versions VARCHAR(500) DEFAULT NULL COMMENT 'Node.js 版本列表（JSON）';
